		...
		if (url.pathname === "/api/chat" && request.method === "POST") {
			try {
				...
				if ((lowerMsg.includes("bedroom") || lowerMsg.includes("master")) && (lowerMsg.includes("light") || lowerMsg.includes("lamp"))) {
					...
					liveContext = `[SYSTEM DIRECTIVE - MANDATORY TOOL EXECUTION] The user wants to control the master bedroom lights. You MUST execute the tool "control_house_lights" with arguments ${argsJson}. Respond with ONE SHORT confirmation sentence only. Do NOT copy prior response text from context. Do NOT write test suite language, alias confirmations, or next test prompts. Then emit the raw trigger payload JSON on its own line at the very end, prefixed with the siren emoji followed by THEATER_ACTION_TRIGGER: — that exact prefix. You MUST emit the trigger JSON. Failure to emit means the lights do NOT change. Do NOT write any bracketed footer text yourself — the system appends the real result after dispatch.` + " [CRITICAL TOOL EMISSION FORMAT REMINDER] Your response must end with the exact trigger payload line and nothing after it. Do NOT write any success footer, JSON success block, Tool executed text, or Hardware bridge text in your response prose. The Worker layer appends the real result footer after the Pi dispatches. If you write fake success theater without emitting a real trigger line at the absolute end of your response, the Worker guardrail will strip your entire response and replace it with a forensic warning.";
