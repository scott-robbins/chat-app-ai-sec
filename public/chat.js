...
if (window.innerWidth < 768) {
    ...
    document.body.classList.add("theme-plain");
    const observer = new MutationObserver(() => {
        if (document.body.classList.contains("theme-fancy")) {
            document.body.classList.remove("theme-fancy");
        }
        if (!document.body.classList.contains("theme-plain")) {
            document.body.classList.add("theme-plain");
        }
    });
    observer.observe(document.body, { attributes: true, attributeFilter: ["class"] });
    console.log("MOBILE GUARD ACTIVE - theme-plain locked, theme-fancy blocked");
}
    ...
    try {
        ...
        if (profileRes.ok) {
            ...
            if (window.innerWidth < 768) {
                ...
                document.body.classList.toggle("theme-plain", data.theme === "plain");
            }
